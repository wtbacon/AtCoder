...
int main() {
    ...
    UnionFind UF;
    UF.init(N);
    rep(i, M) UF.unite(u[i], v[i]);

    // 各連結成分のノードとエッジの数を親ノードを軸にして算出する
    // ノードの数: 各ノードの親ノードが何回出現したか
    // エッジの数: エッジの視点となるノードの親ノードが何回出現したか
    vector<int> cntNodes(N);
    vector<int> cntEdges(N);
    rep(i, N) {
        cntNodes[UF.root(i)] += 1;
    }
    rep(i, M) {
        cntEdges[UF.root(u[i])] += 1;
    }

    if (cntNodes == cntEdges) cout << "Yes" << endl;
    else cout << "No" << endl;
